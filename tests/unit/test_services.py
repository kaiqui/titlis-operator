...
class TestSLOService:
    ...
    def test_reconcile_slo_existing(self, slo_service, sample_slo_spec, mock_datadog_port):
        ...
        existing_slo.tags = ["slo_uid:default:test-service", "managed_by:titlis_operator"]
...
class TestScorecardService:
    ...
    def mock_kubernetes_apis(self):
        ...
        with patch('src.application.services.scorecard_service.get_k8s_apis') as mock_get_apis:
            mock_core = Mock()
            mock_apps = Mock()
            mock_custom = Mock()
            ...
            with patch('src.application.services.scorecard_service.client.AutoscalingV2Api') as mock_autoscaling:
                mock_autoscaling_instance = Mock()
                mock_autoscaling.return_value = mock_autoscaling_instance
                
                # Mock networking API
                with patch('src.application.services.scorecard_service.client.NetworkingV1Api') as mock_networking:
    ...
    def scorecard_service(self, mock_kubernetes_apis):
        ...
        # Mock KubeStateStore
        with patch('src.application.services.scorecard_service.KubeStateStore') as mock_state_store:
    ...
    def test_extract_value_from_resource(self, scorecard_service, sample_deployment_dict):
    ...
    def test_calculate_pillar_scores(self, scorecard_service):
        ...
        from src.domain.models import (
            ValidationResult, ValidationPillar, 
            ValidationSeverity, ValidationRuleType
        )
    ...
    def test_calculate_overall_score(self, scorecard_service):
        ...
        from src.domain.models import (
            PillarScore, ValidationPillar, ValidationResult, ValidationSeverity
        )
    ...
    def test_get_notification_severity(self, scorecard_service):
        ...
        assert scorecard_service.get_notification_severity(critical_scorecard) == "critical"
        ...
        assert scorecard_service.get_notification_severity(warning_scorecard) == "warning"
